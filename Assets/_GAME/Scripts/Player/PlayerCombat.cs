...
public class PlayerCombat : MonoBehaviour, IDamagable<DamageObject> {
	...
	public UnitState m_playerState; //the state of the player
	...
	void Start() {
		...
		m_playerState = GetComponent<UnitState>();
		...
		if (!m_playerState) Debug.LogError ("No m_playerState component found on " + gameObject.name);
	...
	void Update() {
		...
		if(!isDead && isGrounded && DefendStates.Contains(m_playerState.currentState)) Defend(inputManager.isDefendKeyDown());
	...
	private void CombatInputEvent(INPUTACTION action) {
		...
		if (AttackStates.Contains (m_playerState.currentState) && !isDead) {
			...
			//ground punch
			if (action == INPUTACTION.PUNCH && (m_playerState.currentState != UNITSTATE.PUNCH && NearbyEnemyDown()) && isGrounded) {
			...
			//default punch
			if (action == INPUTACTION.PUNCH && m_playerState.currentState != UNITSTATE.PUNCH && m_playerState.currentState != UNITSTATE.KICK && isGrounded) {
			...
			if (action == INPUTACTION.PUNCH && (m_playerState.currentState == UNITSTATE.PUNCH) && !continuePunchCombo && isGrounded) {
			...
			//ground kick
			if (action == INPUTACTION.KICK && (m_playerState.currentState != UNITSTATE.KICK && NearbyEnemyDown()) && isGrounded) {
				if(GroundKickData.animTrigger.Length > 0) doAttack(GroundKickData, UNITSTATE.GROUNDKICK, INPUTACTION.KICK);
				return;
			}

			//default kick
			if (action == INPUTACTION.KICK && m_playerState.currentState != UNITSTATE.KICK && m_playerState.currentState != UNITSTATE.PUNCH && isGrounded) {
			...
			if (action == INPUTACTION.KICK && (m_playerState.currentState == UNITSTATE.KICK) && !continueKickCombo && isGrounded) {
	...
	private void doAttack(DamageObject d, UNITSTATE state, INPUTACTION inputAction){
		...
		//_photonView.RPC("RPC_doAttckAnim",RpcTarget.All, d);
		m_playerState.SetState(state);
		lastAttack = d;
	...
	// [PunRPC]
	// public void RPC_doAttckAnim(DamageObject d)
	// {
	// 	animator.SetAnimatorTrigger(d.animTrigger);
	// }
	...
	void useCurrentWeapon(){
		m_playerState.SetState (UNITSTATE.USEWEAPON);
		TurnToDir(currentDirection);
		SetVelocity(Vector3.zero);
	...
	IEnumerator JumpKickInProgress(){
		...
		//_photonView.RPC("RPC_JumpKickAnim",RpcTarget.All);
		...
		//check for hit
		while (m_playerState.currentState == UNITSTATE.JUMPKICK) {
	...
	private void Defend(bool defend){
		...
		if (defend) {
			...
			SetVelocity(Vector3.zero);
			m_playerState.SetState (UNITSTATE.DEFEND);
		} else {
			m_playerState.SetState (UNITSTATE.IDLE);
		}
	}

	// [PunRPC]
	// public void RPC_DefendAnim(bool defend)
	// {
	// 	animator.SetAnimatorBool("Defend", defend);
	// }
	...
	public void Hit(DamageObject d) {
		...
		if (HitableStates.Contains (m_playerState.currentState)) {
			CancelInvoke();
			...
			if(m_playerState.currentState == UNITSTATE.DEFEND && !d.DefenceOverride && (isFacingTarget(d.inflictor) || blockAttacksFromBehind)) {
				Defend(d);
				return;
			} else {
				animator.SetAnimatorBool("Defend", false);
				//_photonView.RPC("RPC_Defend", RpcTarget.All);
			}
			...
			//check for knockdown
			if ((hitKnockDownCount >= knockdownHitCount || !IsGrounded() || d.knockDown) && m_playerState.currentState != UNITSTATE.KNOCKDOWN) {
				hitKnockDownCount = 0;
			...
			//_photonView.RPC("RPC_Hit", RpcTarget.All, i);
			SetVelocity(Vector3.zero);
			m_playerState.SetState (UNITSTATE.HIT);

			//add a small force from the impact
			if (isFacingTarget(d.inflictor)) { 
				animator.AddForce (-1.5f);
				//_photonView.RPC("RPC_AddForce", RpcTarget.All, -1.5f);
			} else {
				animator.AddForce (1.5f);
				//_photonView.RPC("RPC_AddForce", RpcTarget.All, 1.5f);
			}
	...
	// [PunRPC]
	// public void RPC_Defend()
	// {
	// 	animator.SetAnimatorBool("Defend", false);
	// }
	//
	// [PunRPC]
	// public void RPC_Hit(int i)
	// {
	// 	animator.SetAnimatorTrigger ("Hit" + i);
	// }
	//
	// [PunRPC]
	// public void RPC_AddForce(float f)
	// {
	// 	animator.AddForce (f);
	// }
	...
	void Defend(DamageObject d){
		...
		//_photonView.RPC("RPC_AddforceAnim", RpcTarget.All, -hitKnockBackForce);
		} else {
			animator.AddForce (hitKnockBackForce);
			//_photonView.RPC("RPC_AddforceAnim", RpcTarget.All, hitKnockBackForce);
		}
	...
	// [PunRPC]
	// public void RPC_AddforceAnim(float f)
	// {
	// 	animator.AddForce(f);
	// }
	...
	public void interactWithItem(){
		if (itemInRange != null){
			...
			//_photonView.RPC("RPC_Pickup", RpcTarget.All);
			m_playerState.SetState(UNITSTATE.PICKUPITEM);
			SetVelocity(Vector3.zero);
			Invoke ("Ready", .3f);
	...
	public void equipWeapon(Weapon weapon){
		...
		//_photonView.RPC("RPC_Weapon", RpcTarget.All, weapon);
	...
	public IEnumerator KnockDownSequence(GameObject inflictor) {
		m_playerState.SetState (UNITSTATE.KNOCKDOWN);
		...
		m_playerState.currentState = UNITSTATE.STANDUP;

		yield return new WaitForSeconds (KnockdownStandUpTime);
		m_playerState.currentState = UNITSTATE.IDLE;
	}
